 ...
 */
$pdf->AddPage();
	$pdf->Image("pdfimg/page1.jpg", 0, 0, 8, 11, "JPG");
 ...
 */
$pdf->AddPage(); 
	$pdf->Image("pdfimg/page3.jpg", 0.5, 0, 7, 11, "JPG");
...
$pdf->AddPage(); 
	//back ground
	$pdf->Image("pdfimg/page2.jpg", 1, 0, 7, 11, "JPG");
	...
	//run category checkbox
		...
		$pdf->Image("pdfimg/check.png", 1.35, 0.85, 0.4, 0.3, "PNG");
		//check for 3k (children)
		if($rows['race_category'] == '3k-children')
		$pdf->Image("pdfimg/check.png", 1.90, 0.85, 0.4, 0.3, "PNG");
		//check for 3k (adult)
		if($rows['race_category'] == '3k-adult')
		$pdf->Image("pdfimg/check.png", 2.92, 0.85, 0.4, 0.3, "PNG");
		//check for 3k (family)
		if($rows['race_category'] == '3k-family')
		$pdf->Image("pdfimg/check.png", 3.84, 0.85, 0.4, 0.3, "PNG");
		//check for 5k
		if($rows['race_category'] == '5k-adult')
		$pdf->Image("pdfimg/check.png", 4.84, 0.85, 0.4, 0.3, "PNG");
		//check for 10k
		if($rows['race_category'] == '10k-adult')
		$pdf->Image("pdfimg/check.png", 5.42, 0.85, 0.4, 0.3, "PNG");
	...
	//are you employed by mcdonals philippines
		...
		//yes
		$pdf->Image("pdfimg/check.png", 1.24, 5.36, 0.4, 0.3, "PNG");
			//department
			$pdf->Text(1.8 , 5.6 , substr($rows['mcdonalds_employed'],6,500) );
		}else{
			//no
			$pdf->Image("pdfimg/check.png", 1.24, 5.66, 0.4, 0.3, "PNG");
		}
	//do you have any family members joining with you
		if($rows['race_category'] == '3k-family'){
			//yes
			$pdf->Image("pdfimg/check.png", 1.24, 6.45, 0.4, 0.3, "PNG");
			...
			//no
			$pdf->Image("pdfimg/check.png", 1.22, 8.22, 0.4, 0.3, "PNG");
		}
	//would you like to recieve about mcdonals promotion
		if($rows['news_letter'] == 'checked'){
			//yes 
			$pdf->Image("pdfimg/check.png", 1.22, 8.98, 0.4, 0.3, "PNG");
				//new products
				$pdf->Image("pdfimg/check.png", 1.47, 9.23, 0.4, 0.3, "PNG");
				//new promotional items
				$pdf->Image("pdfimg/check.png", 1.47, 9.4, 0.4, 0.3, "PNG");
				//happy meals
				$pdf->Image("pdfimg/check.png", 1.47, 9.6, 0.4, 0.3, "PNG");
				//family events
				$pdf->Image("pdfimg/check.png", 1.47, 9.77, 0.4, 0.3, "PNG");
				//how would you live to be reached by this information
				//email
				$pdf->Image("pdfimg/check.png", 1.2, 10.25, 0.4, 0.3, "PNG");
				//mobile number
				$pdf->Image("pdfimg/check.png", 1.2, 10.43, 0.4, 0.3, "PNG");
				//home address
				$pdf->Image("pdfimg/check.png", 1.2, 10.63, 0.4, 0.3, "PNG");
		}else{
			//no
			$pdf->Image("pdfimg/check.png", 4.62, 8.98, 0.4, 0.3, "PNG");
		}
 ...
 */
$pdf->AddPage(); 
	$pdf->Image("pdfimg/page4.jpg", 0.8, 0, 7, 11, "JPG");
